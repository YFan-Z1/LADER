...
from model_ori.model_factory_final import get_model
import pandas as pd
...
class Evaluator:
    """
    Evaluator class, adapted from: https://github.com/Tushar-N/attributes-as-operators
    With modifications from: https://github.com/ExplainableML/czsl
    """

    def __init__(self, dset, model, device=None):
        self.device = device
        self.dset = dset
...
def predict_logits(model, dataset, config):
    ...
    device = torch.device(f"cuda:{config.gpu_index}" if torch.cuda.is_available() else "cpu")
    pairs = torch.tensor([(attr2idx[attr], obj2idx[obj])
                                for attr, obj in pairs_dataset]).to(device)
    dataloader = DataLoader(
        dataset, batch_size=config.eval_batch_size,
        shuffle=False, num_workers=config.num_workers)
    all_logits = torch.Tensor()
    loss = 0
    with torch.no_grad():
        if config.text_first:
            text_feats = model.encode_text_for_open(pairs)
        for idx, data in tqdm(enumerate(dataloader), total=len(dataloader), desc="Testing"):
            predict = model(data, pairs) if not config.text_first else model.forward_for_open(data, text_feats)
            logits = model.logit_infer(predict, pairs)
            loss += model.loss_calu(predict, data, pairs).item()
...
def threshold_with_feasibility(
    ...
    Args:
        logits (torch.Tensor): the cosine similarities between
            the _images and the attribute-object pairs.
...
def test(
    ...
    df = pd.DataFrame({
        'Predicted Attributes': results['predicted_attributes'],
        'Predicted Objects': results['predicted_objects'],
        'True Attributes': results['true_attributes'],
        'True Objects': results['true_objects'],
        'True Pairs': results['true_pairs']
    })
...
if __name__ == "__main__":
    ...
    log_name = 'exp-report.log'
    logger, log_path = setup_logger(config.test_save_path, log_name)
    device = torch.device(f"cuda:{config.gpu_index}" if torch.cuda.is_available() else "cpu")
    config.device = device
    logger.info(f'Using device: GPU-{config.gpu_index}]')
    # set the seed value
    test_type = 'OPEN WORLD' if config.open_world else 'CLOSED WORLD'
    logger.info(f"{test_type} evaluation details for {config.dataset} dataset")
    ...
    model = get_model(config, attributes=attributes, classes=classes, offset=offset, device=device, logger=logger)
    if config.load_model:
        load_model(model, config.load_model)
    ...
    if config.open_world and config.dataset != 'clothing16k':
        feasibility_path = f'data/feasibility_{config.dataset}.pt'
    ...
    with torch.no_grad():
        ...
        if config.open_world and best_th is not None and config.dataset != 'clothing16k':
            print('using threshold: ', best_th)
            all_logits = threshold_with_feasibility(
                all_logits, test_dataset.seen_mask,
                threshold=best_th, feasiblity=unseen_scores)
        test_stats = test(
            test_dataset, evaluator,
            all_logits, all_attr_gt, all_obj_gt, all_pair_gt, config)
