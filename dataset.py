...
def remove_illegal_characters_from_filename(image_path):
    directory, filename = os.path.split(image_path)
    name, extension = os.path.splitext(filename)
    cleaned_name = re.sub(r'[^\w\.\-]', '_', name)
    new_filename = cleaned_name + extension
...
class CompositionDataset(Dataset):
    def __init__(
        ...
        self.loader = ImageLoader(self.root + '/_images/')
    ...
    def get_split_info(self):
        ...
        for instance in data:
            ...
            '''
            if 'mit' in self.root:
                path = os.path.join('data/mit-states/_images/', image)
