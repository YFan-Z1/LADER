...
from model.utils.common_utils import expand_tensor
...
class Stage1LocalAlignment(nn.Module):
    def __init__(self, d_model, num_heads=12, dropout=0.1, num_cmt_layers_first=1):
        ...
        self.cross_attn_layers = nn.ModuleList([
            CrossAttentionLayer(d_model, num_heads, dropout, mlp_ratio=4.0) for _ in range(num_cmt_layers_first)
        ])
...
class Stage2GlobalFusion(nn.Module):
    def __init__(self, d_model, num_heads=12, dropout=0.1, num_cmt_layers_second=3):
        super().__init__()
        self.cross_attn_layers = nn.ModuleList([
            CrossAttentionLayer(d_model, num_heads, dropout, mlp_ratio=4.0) for _ in range(num_cmt_layers_second)
        ])
        self.norm = nn.LayerNorm(d_model)
        self.dropout = nn.Dropout(dropout)
...
class MSCI(ThreeBranch):
    def __init__(self, config, attributes, classes, offset, device, logger):
        ...
        self.cmt = nn.ModuleList([CrossAttentionLayer(output_dim, output_dim // 64, self.cross_attn_dropout, mlp_ratio=4.0) for _ in
    ...
    def encode_image_with_adapter(self, x: torch.Tensor):
        ...
        high_level_features = high_level_features[-1] if self.clip_cfg.selected_high_layers == 1 else self.aggregate_features_high(high_level_features)
        low_level_features = low_level_features[0] if self.clip_cfg.selected_low_layers == 1 else self.aggregate_features_low(low_level_features)
    ...
    def forward(self, batch, idx, return_features=False):
        ...
        b = batch_img.shape[0]
        cls_token, low_level_features, high_level_features = self.encode_image_with_adapter(batch_img.type(self.clip.dtype))
        ...
        for stage in ['pair', 'attr', 'obj']:
            ...
            cmt_text_features = text_feats[stage] + self.lamda * cmt_1.squeeze(1) + self.lamda_2 * cmt_2.squeeze(1)
        ...
        if return_features:
            return logits, (high_level_features, low_level_features, cmt_text_features)
        else:
            return logits
